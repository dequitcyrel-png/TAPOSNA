...
class StudentProjectController extends Controller
{
    ...
    public function finalize(
        ...
        */
        ...
        |--------------------------------------------------------------------------
        */
        $submissionLink = $request->input('submission_link') ?? ($submission->submission_link ?? null);
        if ($submissionLink === '') $submissionLink = null;

        if (
            empty(trim($content ?? '')) &&
            empty($filePath) &&
            empty($submissionLink)
        ) {
            return back()->with(
                'error',
                'Please upload a file, add a link, or write content.'
            );
        }
